...
function EntidadesComModal({ title, items, fetchItemsData, fetchItems, isExternal }) {
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="flex-1 px-4 pt-6 pb-6 overflow-y-auto">
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 lg:gap-4 lg:p-4">
          {items.map((item) => (
            <div
              key={isExternal ? item.id_externo : item.id}
              ...
              onClick={() => handleItemClick(isExternal ? item.id_externo : item.id)}
            >
              <h1>{title} {item.externo?.nome_cliente || item.numero}</h1>
            </div>
          ))}
        </div>
      </div>
...
export default EntidadesComModal;
